...
namespace TaobaoTools.Dialog
{
    public partial class TradeListDlg : UserControl
    {
        ...
        List<TradeInfo> mOtherDeliverTrades = new List<TradeInfo>();
        List<TradeInfo> mNeedPrintTrades = new List<TradeInfo>();
        List<TradeInfo> mNeedExpressPrintTrades = new List<TradeInfo>();
        ...
        void GetTradeList()
        {
            ...
            mTradeList.Sort(new Global.TradeCreatedCompare());
            ...
            toolStripStatusLabelTotal.Text =
                String.Format("[{0}待发货，{1}待出货，{2}待出单，总金额{3}元]",
                mTradeList.Count,
                mOtherDeliverTrades.Count + mNeedPrintTrades.Count,
                mNeedExpressPrintTrades.Count,
                totalPayment);
        }

        void UpdateTradeInfoList()
        {
            mOtherDeliverTrades.Clear();
            mNeedPrintTrades.Clear();
            mNeedExpressPrintTrades.Clear();

            List<TradeInfo> rawTradeInfoList = new List<TradeInfo>();
            foreach (Trade trade in mTradeList)
            {
                // 其它运送方式
                ...
                {
                    // 出货单
                    if (!TopClientHelper.TradePrinted(trade))
                        Global.AddTrade(mOtherDeliverTrades, trade);
                }
                else
                {
                    // 计算出总编号
                    TradeInfo info = Global.AddTrade(rawTradeInfoList, trade);
                    int index = rawTradeInfoList.IndexOf(info) + 1;

                    // 出货单
                    if (!TopClientHelper.TradePrinted(trade))
                        Global.AddTrade(mNeedPrintTrades, trade).Index = index;

                    // 快递单
                    if (!TopClientHelper.ExpressTradePrinted(trade))
                        Global.AddTrade(mNeedExpressPrintTrades, trade).Index = index;
                }
            }
        }
        ...
        void UpdateTradeData(ListViewItem lvItem)
        {
            ...
            String deliverType = (trade.SellerFlag & (long)Global.FlagBits.FB_OtherDeliver) != 0 ?
                "其它" : "";
            String printedText = (trade.SellerFlag & (long)Global.FlagBits.FB_Printed) != 0 ?
                "已打印" : "";
            String expressPrinted = (trade.SellerFlag & (long)Global.FlagBits.FB_ExpressPrinted) != 0 ?
                "已打印" : "";
            ...
            lvItem.SubItems.Add(printedText);
            lvItem.SubItems.Add(expressPrinted);
        ...
        {
            PrintOrderDlg dlg = new PrintOrderDlg(mOtherDeliverTrades, mNeedPrintTrades, DateTime.Now.ToShortDateString());
            dlg.ShowDialog();
        }

        private void OnExpressSettingClicked(object sender, EventArgs e)
        {
            PrintExpressDlg dlg = new PrintExpressDlg(mNeedExpressPrintTrades);
            dlg.ShowDialog();
        }
        ...
        private void OnCancelIngoreCliecked(object sender, EventArgs e)
        {
            ...
            foreach (ListViewItem lvItem in listView1.SelectedItems)
            {
                ...
                if (TopClientHelper.TradePrinted(trade))
                {
                    TopClientHelper.TradePrinted(trade, false);
                    UpdateTradeData(lvItem);
                    changed = true;
                }
            }
        ...
        private void OnIgnoreClicked(object sender, EventArgs e)
        {
            ...
            foreach (ListViewItem lvItem in listView1.SelectedItems)
            {
                ...
                if (!TopClientHelper.TradePrinted(trade))
                {
                    TopClientHelper.TradePrinted(trade, true);
                    UpdateTradeData(lvItem);
                    changed = true;
                }
            }
        ...
        private void OnExpressPrintClicked(object sender, EventArgs e)
        {
            bool changed = false;
            foreach (ListViewItem lvItem in listView1.SelectedItems)
            {
                ...
                if (!TopClientHelper.ExpressTradePrinted(trade))
                {
                    TopClientHelper.ExpressTradePrinted(trade, true);
                    UpdateTradeData(lvItem);
                    changed = true;
                }
            }
        ...
        private void OnExpressNoPrintClicked(object sender, EventArgs e)
        {
            bool changed = false;
            foreach (ListViewItem lvItem in listView1.SelectedItems)
            {
                ...
                if (TopClientHelper.ExpressTradePrinted(trade))
                {
                    TopClientHelper.ExpressTradePrinted(trade, false);
                    UpdateTradeData(lvItem);
                    changed = true;
                }
            }
        ...
        private void ClearFlagClicked(object sender, EventArgs e)
        {
            bool changed = false;
            foreach (ListViewItem lvItem in listView1.SelectedItems)
            {
                ...
                TopClientHelper.ModifyTradeFlag(trade, 0);
                changed = true;
            }
